...
const mockInterviews = new Map<string, any>();
...
export const createApplication = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (isMockMode()) {
    ...
    const actionReceipt = await interactionReceiptService.upsert({
      ...
      actorId: userId,
      status: 'COMPLETED',
      nextStep: 'SPACE_REVIEW',
      requestId: req.requestId,
      payload: { vacancyId: String(vacancyId) },
    });
  ...
  if (!vacancy) {
    return res.status(404).json({ error: 'Vaga não encontrada.', code: 'VACANCY_NOT_FOUND' });
  }
  ...
  if (existing) {
    return res.status(409).json({
      error: 'Você já possui candidatura para esta vaga.',
  ...
  try {
    await interactionService.emitRecruitmentApplication({
      applicationId: application.id,
      candidateId: userId,
      spaceId: vacancy.space_id,
      vacancyTitle: vacancy.title,
    });
  } catch (error) {
    interactionService.logInteractionFailure('recruitment.application.created', error, {
      requestId: req.requestId,
      applicationId: application.id,
    });
  }
...
export const scheduleInterview = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (isMockMode()) {
    const application = mockApplications.get(String(id));
    if (!application) {
      return res.status(404).json({ error: 'Candidatura não encontrada.', code: 'APPLICATION_NOT_FOUND' });
    }
  ...
  const application = await prisma.recruitmentApplication.findUnique({
    where: { id: String(id) },
    include: {
      vacancy: { select: { id: true, title: true, space_id: true } },
    },
  });
  ...
  try {
    await interactionService.emitRecruitmentInterviewInvite({
      interviewId: interview.id,
      spaceId: application.space_id,
      guardianId: interview.guardian_id,
      scheduledFor: interview.scheduled_for.toISOString(),
    });
  } catch (error) {
    interactionService.logInteractionFailure('recruitment.interview.invited', error, {
      requestId: req.requestId,
      interviewId: interview.id,
    });
  }
...
export const respondInterview = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (isMockMode()) {
    const interview = mockInterviews.get(String(id));
    if (!interview) {
      return res.status(404).json({ error: 'Entrevista não encontrada.', code: 'INTERVIEW_NOT_FOUND' });
    }
    const accepted = decision === 'ACCEPT';
    interview.status = accepted ? 'ACCEPTED' : 'DECLINED';
    interview.response_note = note || null;
    interview.responded_at = new Date().toISOString();
    interview.updated_at = new Date().toISOString();
    mockInterviews.set(interview.id, interview);

    const application = mockApplications.get(String(interview.application_id));
    if (application) {
      application.status = accepted ? 'INTERVIEW_ACCEPTED' : 'REJECTED';
    ...
    const actionReceipt = await interactionReceiptService.upsert({
      ...
      actorId,
      status: 'COMPLETED',
      nextStep: accepted ? 'SPACE_DECISION' : 'APPLICATION_CLOSED',
      requestId: req.requestId,
      payload: { decision, note: note || null },
    });

    return res.json({
      code: accepted ? 'INTERVIEW_ACCEPTED' : 'INTERVIEW_DECLINED',
      interview,
      actionReceipt,
    });
  }
  ...
  if (interview.guardian_id !== actorId) {
    return res.status(403).json({ error: 'Sem permissão para responder esta entrevista.', code: 'FORBIDDEN' });
  }
  ...
  await prisma.recruitmentApplication.update({
    ...
    data: { status: accepted ? 'INTERVIEW_ACCEPTED' : 'REJECTED' },
  });

  try {
    await interactionService.emitRecruitmentInterviewResponse({
      interviewId: interview.id,
      guardianId: actorId,
      spaceId: interview.space_id,
      accepted,
    });
  } catch (error) {
    interactionService.logInteractionFailure('recruitment.interview.response', error, {
      requestId: req.requestId,
      interviewId: interview.id,
    });
  }
...
export const decideApplication = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (isMockMode()) {
    ...
    application.decided_by = actorId;
    application.decided_at = new Date().toISOString();
    application.notes = note || application.notes || null;
    application.updated_at = new Date().toISOString();
    mockApplications.set(application.id, application);
    ...
    const actionReceipt = await interactionReceiptService.upsert({
      ...
      actorId,
      status: 'COMPLETED',
      nextStep: decision === 'HIRED' ? 'ONBOARDING' : 'CLOSED',
      requestId: req.requestId,
      payload: { decision },
    });
    return res.json({
      code: decision === 'HIRED' ? 'APPLICATION_HIRED' : 'APPLICATION_REJECTED',
      application,
      actionReceipt,
    });
  }
  ...
  try {
    await interactionService.emitRecruitmentDecision({
      applicationId: application.id,
      actorId,
    ...
    interactionService.logInteractionFailure('recruitment.application.decision', error, {
      requestId: req.requestId,
      applicationId: application.id,
    });
  }
...
export const listApplications = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (isMockMode()) {
    const values = Array.from(mockApplications.values());
    const applications =
      scope === 'candidate' || role === 'PROFESSIONAL' || role === 'CLIENT'
        ? values.filter((application) => String(application.candidate_id) === userId)
        : values.filter((application) => String(application.space_id) === userId);
