...
test.describe('Auth Flow', () => {
  ...
  test('modo teste deve recusar email fora da lista pré-definida', async ({ page }) => {
    await page.goto('/login');
    await page.getByRole('button', { name: /já iniciei a jornada/i }).click();

    await page.fill('input[placeholder="seu@email.com"]', 'qualquer@invalido.com');
    await page.fill('input[placeholder="••••••••"]', '123456');
    await page.click('button[type="submit"]');

    await expect(page.getByText(/apenas e-mails pré-definidos/i)).toBeVisible({ timeout: 10000 });
  });
});
