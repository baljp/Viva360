...
const SettingsHeader: React.FC<{ title: string, onBack: () => void }> = ({ title, onBack }) => (
    <header className="flex-none flex items-center gap-4 px-6 pt-[calc(2rem+env(safe-area-inset-top))] pb-6 bg-white border-b border-nature-100 z-50 shadow-sm">
...
export const SettingsViews: React.FC<SettingsProps> = ({ user, view, setView, updateUser, onLogout }) => {
    ...
    if (view === ViewState.SETTINGS_PROFILE) {
      return (
        <div className="fixed inset-0 z-[200] flex flex-col bg-nature-50 animate-in slide-in-from-right duration-300">
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-8 no-scrollbar pb-32">
                <div className="flex flex-col items-center gap-6">
                  <div className="relative group">
                    <DynamicAvatar user={user} size="xl" className="border-4 border-white shadow-xl" />
                    <button className="absolute bottom-0 right-0 p-3 bg-nature-900 text-white rounded-2xl shadow-lg active:scale-90 transition-all border-2 border-white">
                      <Camera size={18} />
                    </button>
                  </div>
                </div>
                ...
                <button 
                  ...
                  className="w-full py-5 bg-nature-900 text-white rounded-3xl font-bold uppercase tracking-widest text-[11px] shadow-2xl active:scale-95 transition-all"
                >
                  Ancorar Alterações
                </button>
            </div>
    ...
    return (
        <div className="flex flex-col animate-in fade-in h-full pb-48 px-2 overflow-y-auto no-scrollbar">
