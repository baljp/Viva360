...
const SensorySearchEngine: React.FC<ClientProps & { onViewDetails?: (pro: Professional) => void, onToggleFavorite?: (id: string) => void }> = ({ onViewDetails, onToggleFavorite, user }) => {
    ...
    const performSearch = (criteria: string) => {
        ...
        setTimeout(() => {
            ...
            const filtered = MOCK_PROS.filter(p => 
                ...
                p.specialty.some(s => s.toLowerCase().includes(criteriaLower)) ||
                p.bio.toLowerCase().includes(criteriaLower)
            );
            setResults(filtered.length > 0 ? filtered : MOCK_PROS); // Fallback to all pros for demo if empty
                            ...
                            return (
                                <div key={pro.id} className="relative bg-white p-4 rounded-[2.5rem] border border-nature-100 shadow-sm transition-all hover:shadow-md hover:border-primary-100 group">
                                    ...
                                    <button
                                        ...
                                        className={`absolute top-5 right-5 z-20 p-2.5 rounded-full backdrop-blur-md transition-all duration-300 ${
                                            isFav 
...
// --- MAIN EXPORT ---
export const ClientViews: React.FC<ClientProps & { view: ViewState, onSelectPro: (p: Professional) => void, selectedPro: Professional | null, onBookingComplete: () => void }> = (props) => {
    switch (props.view) {
        // Search & Discovery (REPLACED)
        case ViewState.CLIENT_SEARCH: return <SensorySearchEngine {...props} onViewDetails={props.onViewDetails} onToggleFavorite={props.onToggleFavorite} />;
        ...
        case ViewState.CLIENT_BOOKING_FLOW: 
            if (!props.selectedPro) return <SensorySearchEngine {...props} onViewDetails={props.onViewDetails} />; 
