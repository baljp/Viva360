        ...
        <div className="flex flex-col h-full bg-[#fcfdfc] animate-in fade-in">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 pb-32">
                ...
                <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                    {['Todos', 'Contratos', 'Financeiro', 'Salas', 'Equipe'].map((tab, i) => (
                        <button key={tab} className={`px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-widest whitespace-nowrap border ${i === 0 ? 'bg-nature-900 text-white border-nature-900' : 'bg-white text-nature-400 border-nature-100'}`}>
                            {tab}
                        </button>
                    ))}
                </div>
                ...
                <div className="p-8 bg-indigo-50 rounded-[3rem] border border-indigo-100 flex flex-col items-center text-center gap-4 border-dashed mt-8">
                     ...
                     <button className="px-6 py-3 bg-indigo-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg active:scale-95 transition-all">
                         Exportar Log (PDF)
                     </button>
                </div>
            </div>
