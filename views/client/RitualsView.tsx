...
export const RitualsView: React.FC<{ user: User, updateUser: (u: User) => void, onClose?: () => void }> = ({ user, updateUser, onClose }) => {
    ...
    const handleComplete = async (habitId: string) => {
        const reward = gardenService.calculateRitualReward(user);
        const updatedUser = {
            ...user,
    ...
    const renderRoutineSection = (title: string, icon: React.ReactNode, period: 'morning' | 'night', steps: RoutineStep[]) => {
        ...
        return (
            <div key={period} className={`bg-white p-6 rounded-[2.5rem] border transition-all duration-500 shadow-sm space-y-4 ${isAllDone ? 'border-emerald-200 shadow-emerald-100' : 'border-nature-100'}`}>
